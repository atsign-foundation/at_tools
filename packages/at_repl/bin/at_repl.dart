...
import 'package:at_repl/version.dart' as version;
...
Future<void> main(List<String> args) async {
  ...
  final AtClient atClient = cliBase.atClient;
  final String atSign = cliBase.atSign;

  final bool verbose = cliBase.verbose;

  //Define logger and REPL.
  AtSignLogger.root_level = verbose ? 'info' : 'warning';
  final REPL repl = REPL(atClient: atClient);
      ...
      if (command.isNotEmpty) {
        ...
        if (inMonitorMode) {
          if (command.toLowerCase() == 'q' ||
            ...
            stdout.writeln(yellow.wrap(
                "Monitor mode is active. Use 'q', 'quit', or 'stop' to exit monitor mode."));
            continue;
          }
        }
        ...
        if (inNotifyInteractiveMode) {
          if (notifyWaitingForAction && selectedNotifyIndex != null) {
            ...
            if (action == 'v' ||
                ...
                action == 'd' ||
                action == 'delete') {
              bool shouldRefresh = await repl.handleNotifyAction(
                  action, notifyInteractiveList, selectedNotifyIndex);
              ...
              stdout.writeln(yellow
                  .wrap("Invalid action. Use 'v' to view or 'd' to delete."));
              selectedNotifyIndex = null;
              stdout.writeln(lightBlue.wrap(
                  "\nEnter notification index (1-${notifyInteractiveList.length}), 'l' to refresh list, or 'q' to quit:"));
              continue;
          ...
          if (command.toLowerCase() == 'q' || command.toLowerCase() == 'quit') {
            stdout.writeln(
                lightGreen.wrap("Exiting notification inspect mode..."));
            inNotifyInteractiveMode = false;
            stdout.write(magenta.wrap("$atSign "));
            continue;
          }
          ...
          if (index != null &&
              index >= 1 &&
              index <= notifyInteractiveList.length) {
            ...
            stdout.writeln(lightCyan
                .wrap("Selected notification #$index from $from to $to"));
            stdout.writeln(lightBlue.wrap("Choose action: (v)iew or (d)elete"));
            notifyWaitingForAction = true;
            continue;
          } else {
            stdout.writeln(yellow.wrap(
                "Invalid selection. Please enter a number between 1 and ${notifyInteractiveList.length}, 'l' to refresh list, or 'q' to quit."));
            stdout.writeln(lightBlue.wrap(
                "\nEnter notification index (1-${notifyInteractiveList.length}), 'l' to refresh list, or 'q' to quit:"));
            continue;
          }
        ...
        if (inInteractiveMode) {
          if (waitingForAction && selectedKey != null) {
            ...
            if (action == 'v' || action == 'view') {
              try {
                ...
                stdout
                    .writeln(red.wrap("Error getting value: ${e.toString()}"));
              }
              ...
              try {
                ...
                interactiveAtKeys =
                    await atClient.getAtKeys(regex: interactiveRegex);
                stdout.writeln(lightGreen.wrap("AtKey deleted successfully."));

                if (interactiveAtKeys.isEmpty) {
                  if (interactiveRegex.isNotEmpty) {
                    stdout.writeln(yellow.wrap(
                        "No more AtKeys found matching regex '$interactiveRegex'. Exiting inspect mode."));
                  } else {
                    stdout.writeln(yellow
                        .wrap("No more AtKeys found. Exiting inspect mode."));
                  }
                ...
                if (interactiveRegex.isNotEmpty) {
                  stdout.writeln(lightGreen.wrap(
                      "Updated AtKeys list: ${interactiveAtKeys.length}/${allAtKeys.length} keys shown with regex '$interactiveRegex'"));
                } else {
                ...
                for (int i = 0; i < interactiveAtKeys.length; i++) {
                  stdout
                      .writeln("${i + 1}. ${interactiveAtKeys[i].toString()}");
                }
              } catch (e) {
              ...
              stdout.writeln(yellow
                  .wrap("Invalid action. Use 'v' for view or 'd' for delete."));
            }
          ...
          if (index != null &&
              index >= 1 &&
              index <= interactiveAtKeys.length) {
            selectedKey = interactiveAtKeys[index - 1];
            stdout
                .writeln(lightCyan.wrap("Selected: ${selectedKey.toString()}"));
            stdout.writeln(lightBlue.wrap("Choose action: (v)iew or (d)elete"));
            waitingForAction = true;
            continue;
          } else {
            stdout.writeln(yellow.wrap(
                "Invalid selection. Please enter a number between 1 and ${interactiveAtKeys.length}, or 'q' to quit."));
            stdout.writeln(lightBlue.wrap(
                "\nEnter the number of the AtKey you want to interact with (or 'q' to quit):"));
            continue;
          }
