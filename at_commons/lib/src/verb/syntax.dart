class VerbSyntax {
  ...
  static const update_meta =
      r'^update:meta:((?:public:)|((?<forAtSign>@?[^@\s]*):))?(?<atKey>((?!:{2})[^@])+)@(?<atSign>[^@:\s]*)(:ttl:(?<ttl>\d+))?(:ttb:(?<ttb>\d+))?(:ttr:(?<ttr>\d+))?(:ccd:(?<ccd>true|false))?(:isBinary:(?<isBinary>true|false))?(?:sharedKeyStatus:(?<sharedKeyStatus>[^:@]+):)?(:isEncrypted:(?<isEncrypted>true|false))?$';
  static const delete =
